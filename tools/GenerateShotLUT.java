...
/**
 * Build-time tool that pre-generates the shot lookup table by running the same
 * RK4 projectile physics that ProjectileSimulator uses. The output is a Java
 * source file that can be compiled into the robot project, eliminating the
 * multi-minute startup delay on the roboRIO.
 *
 * <p>Usage: java GenerateShotLUT <output-file-path>
 *
 * <p>If you change physics parameters (ball mass, drag, hood angles, etc.),
 * update them here AND in AimingCalculator/Constants so both stay in sync.
 */
public class GenerateShotLUT {
    ...
    /** Binary search for RPM at a given angle that lands at target height. Returns {rpm, tof, reachable}. */
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(outFile))) {
            ...
            pw.println(" *");
            pw.printf (" * <p>Generated: %d entries, %.1fs generation time.%n", entries.size(), elapsed / 1000.0);
            pw.println(" *");
            ...
            for (double[] e : entries) {
                pw.printf("        lut.put(%.2f, new ShotParameters(%.1f, %.1f, %.4f));%n",
                        e[0], e[1], e[2], e[3]);
            }
        ...
        System.out.printf("[GenerateShotLUT] %d entries, %.2fm to %.2fm, generated in %.1fs%n",
                entries.size(),
