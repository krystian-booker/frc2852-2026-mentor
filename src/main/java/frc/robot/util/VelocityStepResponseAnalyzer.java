...
/**
 * Records velocity-vs-time samples during step tests and computes step response metrics:
 * rise time, overshoot, settling time, steady-state error, and disturbance recovery time.
 */
public class VelocityStepResponseAnalyzer {
     ...
     */
    public VelocityStepResponseAnalyzer(double startVelocity, double targetVelocity, double tolerance) {
    ...
    /**
     * Time until velocity stays within tolerance of target permanently (seconds).
     * Returns -1 if not settled by end of recording.
     */
    ...
    /**
     * Time from a disturbance until velocity re-enters the tolerance band (seconds).
     * Searches from the given timestamp forward for the first sample back within tolerance.
     *
