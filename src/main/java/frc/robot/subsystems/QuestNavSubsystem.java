...
public class QuestNavSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "QuestNav/Battery",
        this.questNav.getBatteryPercent().isPresent()
            ? this.questNav.getBatteryPercent().getAsInt()
            : 0);
  }

  /**
   * Reset the QuestNav pose to match a known robot position. Call this when resetting odometry
   * (e.g., at start of auto).
   */
  ...
  /**
   * Attempts to immediately seed QuestNav pose from vision. Requires at least 2 visible tags for
   * accurate seeding.
   *
  ...
  /**
   * Creates an instant command that attempts to seed QuestNav from vision. Does nothing if no valid
   * vision pose is available.
   *
